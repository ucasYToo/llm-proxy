 ...
 */
export const assembleAnthropicResponse = (
  events: unknown[],
 ...
 */
export const assembleOpenAIResponse = (
  events: unknown[],
): Record<string, unknown> => {
    ...
    for (const choice of choices) {
      ...
      if (delta) {
        ...
        if (delta.tool_calls) {
          ...
          const existingCalls = accMsg.tool_calls as Array<
            Record<string, unknown>
          >;
          ...
          for (const tc of newCalls) {
            ...
            if (!existingCalls[tcIdx]) {
              ...
              const fn = existingCalls[tcIdx].function as
                | Record<string, unknown>
                | undefined;
              ...
              if (fn) {
                ...
                if (typeof fn.arguments === "string") {
                  existingFn.arguments =
                    ((existingFn.arguments as string) ?? "") + fn.arguments;
                }
