...
- 修复 Codex Dashboard 在工具 Hook 密集时反复全量读取大 payload，导致请求堆叠、页面卡顿和内存瞬时接近 1 GB 的问题；时间线与 SSE 现在使用轻量摘要，完整 Hook 详情按需读取。
- Codex 会话汇总改为 SQLite 聚合查询，Hook 历史新增 128 MB 字节预算，Rollout Trace 时间线改为有界读取，避免日志体积直接放大运行内存。
